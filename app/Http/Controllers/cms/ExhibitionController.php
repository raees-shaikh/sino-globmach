...
class ExhibitionController extends Controller
{
    public function index()
    {
        ...
        return view('cms.exhibition.index', compact('exhibitions'));
    }


    public function create()
    {
        return view('cms.exhibition.create');
    }

    public function show($id)
    {
        $exhibition = Exhibition::findOrFail($id);
        return view('cms.exhibition.show', compact('exhibition'));
    }

    public function store(Request $request)
    {
        $request->validate([
            'title' => 'required|min:3|max:150|unique:exhibitions,title',
        ...
        if ($fileWithExtension) {
            ...
            $destinationPath = storage_path('app/public/images/exhibition/');
            $img = Image::make($fileWithExtension->getRealPath())->resize(600, null, function ($constraint) {
    ...
    {
        $exhibition = Exhibition::findOrFail($id);
        return view('cms.exhibition.edit', compact('exhibition'));
    }

    public function update(Request $request, $id)
    {
        $request->validate([
            'title' => 'required|min:3|max:150|unique:exhibitions,title,' . $id,
    ...
    public function indexMedia($id)
    {
        ...
        return view('cms.exhibition_section.index', compact('medias', 'exhibitions'));
    }

    public function createMedia($id)
    {
        $exhibition = Exhibition::findOrFail($id);
        return view('cms.exhibition_section.create', compact('exhibition'));
    }
    ...
    public function storeMedia(Request $request, $id)
    {
        ...
        foreach ($files as $file) {
            $file_details = uploadFile($file, 'images/exhibition');
            $media = new Media();
            $media->name = 'Gallery';
    ...
    public function editMedia($exhibition_id, $media_id)
    {
        ...
        // dd($medias);
        return view('cms.exhibition_section.edit', compact('exhibition', 'medias'));
    }
