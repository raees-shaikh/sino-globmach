...
@section('pageTitle', ucfirst($subCatOne->name . ' - ' . $category->title . ' - Latest Proposals'))
@section('content')
    <style>
    ...
    <section id="banner-section" class="bg-prefix w-100"
        style='background-image:  url("{{ asset('frontend/mountain-images/mount-' . $subCatOne->id % 18 . '.jpg') }}"); background-attachment: fixed; background-repeat: no-repeat;background-size: cover; background-position:bottom'>
        <div class="overlay">
            <div class="container">
                <div class="row text-center mt-100-px pt-5">
                    <div class="col-md-12 mt-30">
                        <h1 class="lowercase text-white ">{{ $subCatOne->name }}</h1>
                    </div>
                    <div class="col-md-12 mt-2 divide">
                        <ul class="d-flex justify-content-center flex-wrap-links">
                            ...
                            <li><a href="#!" class="text-primary font-sm">{{ $subCatOne->name }}</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-7">
                    @if ($subCatTwo->count() || $offers->count())
                        <div class="blog-grid mt-25">
                            <div class="row">
                                @forelse($subCatTwo as $subCat)
                                    <div class="col-lg-5 mt-4">
                                        <div class="card h-100">
                                            <div class="card-body pt-2 pb-0">
                                                <a
                                                    href="{{ route('frontend.offer.sub_category_three', ['category' => $category->slug, 'sub_category_one' => $subCatOne->slug, 'sub_category_two' => $subCat->slug]) }}">
                                            ...
                                            <div class="blog-btn mt-10">
                                                <a class="btn text-white bg-red"
                                                    href="{{ route('frontend.offer.sub_category_three', ['category' => $category->slug, 'sub_category_one' => $subCatOne->slug, 'sub_category_two' => $subCat->slug]) }}">Read
                                                    More</a>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="d-flex justify-content-center mt-5 mb-3">
                            {{ $subCatTwo->withQueryString()->onEachSide(1)->links('pagination::bootstrap-4') }}
                        </div>
                        @if ($subCatTwo->count() && $offers->count())
                            <hr>
                        @endif
