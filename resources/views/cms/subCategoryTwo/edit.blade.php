        ...
        <div class="container-fluid">
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h4 class="text-themecolor m-0">Edit Sub Category Two </h4>
                </div>
                    ...
                    <div class="card card-body col-lg-6 py-0 pt-2">
                        <div class="row">
                            <div class="col-sm-12 col-xs-12 p-0">
                                <form id="form" action="{{ route('cms.subCategoryTwo.update', $subCategoryTwo->id) }}"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="category_id" id="sel1"
                                            ...
                                            @else
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}"
                                                        @if ($subCategoryTwo->category_id == $category->id) {{ 'selected' }} @endif>
                                                        {{ $category->title }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    ...
                                    <div class="form-group">
                                        <label>Name</label>
                                        <input class="form-control" value="{{ old('name', $subCategoryTwo->name) }}"
...
@section('js')
    <script>
        function getValues() {
            const sub_category_one_id = '{{ $subCategoryTwo->sub_category_one_id ?? null }}';
